...
class ICLearning(nn.Module):
    ...
    num_outputs : int, default=1
        Number of output dimensions. When > 1, creates separate y_encoder and decoder
        for each output dimension, sharing a single tf_icl transformer. The ColEmbedding
        stage uses joint multi-target projection (``Linear(num_outputs, embed_dim)``)
        so that the Set Transformer can learn cross-column attention patterns informed
        by the full target vector. The ICL stage then predicts each dimension independently.
    """
    ...
    def __init__(
        ...
        if num_outputs > 1:
            # Per-dimension y_encoders and decoders (e.g., 12 for RL tasks).
            # Weights are initialized randomly; the caller (TabICL) is responsible
            # for copying pretrained single-output weights into each slot.
            if max_classes > 0:  # Classification
                self.y_encoders = nn.ModuleList([OneHotAndLinear(max_classes, d_model) for _ in range(num_outputs)])
            else:  # Regression
    ...
    def _icl_predictions(self, R: Tensor, y_train: Tensor) -> Tensor:
        ...
        y_train : Tensor
            Training targets of shape (B, train_size) for single-output, or
            (B, train_size, num_outputs) for multi-output.

        Returns
        -------
        Tensor
            - For regression (max_classes=0), single-output:
                shape (B, T, out_dim) where out_dim = num_quantiles
            - For regression (max_classes=0), multi-output (num_outputs > 1):
                shape (B, T, num_outputs, out_dim)
            - For classification (max_classes>0): shape (B, T, out_dim) where out_dim = max_classes
        """

        train_size = y_train.shape[1]

        if self.num_outputs == 1:
            # ── Original single-output path ──
            ...
            # ── Multi-output path: per-dimension ICL ──
            # y_train shape: (B, train_size, num_outputs)
            # R is shared across all dimensions
            assert not self.max_classes > 0, "Multi-output only supported for regression."
            outs = []
            for j in range(self.num_outputs):
                R_j = R.clone()
                y_j = y_train[:, :, j]  # (B, train_size)
                Ry_train = self.y_encoders[j](y_j.unsqueeze(-1))  # (B, train_size, d_model)
                R_j[:, :train_size] = R_j[:, :train_size] + Ry_train
                src_j = self.tf_icl(R_j, train_size=train_size)
                if self.norm_first:
                    src_j = self.ln(src_j)
                out_j = self.decoders[j](src_j)  # (B, T, out_dim)
                outs.append(out_j)
            return torch.stack(outs, dim=-2)  # (B, T, num_outputs, out_dim)
