        ...
        # 从tabicl.py得出默认target_aware=true
            ...
            if not needs_mixed_radix:
                ...
                if self.max_classes > 0:
                    ...
                    # Regression: y_train shape is (..., train_size) for num_targets=1,
                    # or (..., train_size, num_targets) for num_targets > 1.
                    # Linear(num_targets, embed_dim) handles both cases directly.
                    y_emb = self.y_encoder(y_train) if self.num_targets > 1 else self.y_encoder(y_train.unsqueeze(-1))
