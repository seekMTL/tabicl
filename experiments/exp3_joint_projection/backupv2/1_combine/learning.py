...
class ICLearning(nn.Module):
    ...
    def __init__(
        ...
        # y_encoder 和 decoder 在单/多输出下共享同一份权重。
        # 多输出时，每维度传入不同的 y 值，但 y_encoder/decoder 层本身复用。
    ...
    def _icl_predictions(self, R: Tensor, y_train: Tensor) -> Tensor:
        ...
        y_train : Tensor
            ...
            (B, train_size, num_outputs) for multi-output. The train_size is the position
        ...
        if self.num_outputs == 1:
            ...
            for j in range(self.num_outputs):
                ...
                Ry_train = self.y_encoder(y_j.unsqueeze(-1))  # (B, train_size)->(B, train_size, 1)->(B, train_size, d_model) 共享 y_encoder
                R_j[:, :train_size] = R_j[:, :train_size] + Ry_train
                src_j = self.tf_icl(R_j, train_size=train_size)
                if self.norm_first:
                    src_j = self.ln(src_j)
                out_j = self.decoder(src_j)  # 共享 decoder, (B, T, out_dim)
                outs.append(out_j.cpu())  # 移 CPU 防全量 T 12 维叠加 OOM
            result = torch.stack(outs, dim=-2)  # (B, T, num_outputs, out_dim) on CPU
            return result.to(R.device)
