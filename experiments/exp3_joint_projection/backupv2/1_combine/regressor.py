...
class TabICLRegressor(RegressorMixin, TabICLBaseEstimator):
    ...
    def _load_model(self) -> None:
        ...
        if self.num_outputs > 1:
            ...
            # ColEmbedding y_encoder: Linear(1,128) → Linear(num_outputs,128)
            cw = state_dict['col_embedder.y_encoder.weight']  # (128, 1)
            cb = state_dict['col_embedder.y_encoder.bias']    # (128,)
            self.model_.col_embedder.y_encoder.weight.data.copy_(
                cw.repeat(1, self.num_outputs) * (1.0 / self.num_outputs))
            self.model_.col_embedder.y_encoder.bias.data.copy_(cb)
        else:
            # ── 单输出模式：原始加载逻辑 ──
