...
class ColEmbedding(nn.Module):
    ...
    num_targets : int, default=1
        Number of target dimensions. For scalar regression tasks this is 1.
        For multi-output tasks (e.g., RL with [reward, delta_state_0, ..., delta_state_10]),
        set to the total number of target dimensions (e.g., 12). When > 1, the y_encoder is
        ``nn.Linear(num_targets, embed_dim)``, projecting the full target vector jointly into
        the embedding space, allowing the Set Transformer to learn joint column attention patterns.
    ...
    def __init__(
        ...
        if target_aware:
            if max_classes > 0:  # Classification
                assert num_targets == 1, "num_targets > 1 is only supported for regression tasks."
                self.y_encoder = OneHotAndLinear(max_classes, embed_dim)
            else:  # Regression
                if num_targets > 1:
                    # 分组建模: reward(dim 0) 单独一个 y_encoder，其余 delta 共享另一个
                    # 避免量级差异(如 reward~1, delta~0.001)在同一 Linear 中相互淹没
                    self.y_encoder_reward = nn.Linear(1, embed_dim)
                    self.y_encoder_delta = nn.Linear(num_targets - 1, embed_dim)
                else:
        ...
        # 从tabicl.py得出默认target_aware=true
            ...
            if not needs_mixed_radix:
                ...
                if self.max_classes > 0:
                    ...
                    # 分组建模(num_targets>1): reward(dim 0) 和 delta(dims 1..) 分别编码后相加
                    # 避免量级差异导致信号淹没
                    if self.num_targets > 1:
                        y_reward = y_train[..., 0:1]   # (..., train_size, 1)
                        y_delta = y_train[..., 1:]     # (..., train_size, num_targets-1)
                        y_emb = self.y_encoder_reward(y_reward) + self.y_encoder_delta(y_delta)
                    else:
    ...
    def _train_forward_with_feature_group(self, X: Tensor, y_train: Tensor, embed_with_test: bool) -> Tensor:
        ...
        if self.target_aware:
            assert y_train is not None, "y_train must be provided when target_aware=True."
            y_train = y_train.unsqueeze(1).expand(-1, features.shape[1], *([-1] * (y_train.dim() - 1)))
    ...
    def _train_forward_without_feature_group(
        ...
        if d is None:
            ...
            if self.target_aware:
                ...
                y_train = y_train.unsqueeze(1).expand(-1, HC, *([-1] * (y_train.dim() - 1)))  # (B, H+C, train_size)
    ...
    def _compute_embeddings_with_cache(
        ...
        if not self.target_aware:
            ...
            if store_cache:
                ...
                if self.max_classes > 0:
                    y_emb = self.y_encoder(y_train.float())
                else: # 回归: 分组建模(num_targets>1) 或单输出
                    if self.num_targets > 1:
                        y_reward = y_train[..., 0:1]
                        y_delta = y_train[..., 1:]
                        y_emb = self.y_encoder_reward(y_reward) + self.y_encoder_delta(y_delta)
                    else:
                        y_emb = self.y_encoder(y_train.unsqueeze(-1))
    ...
    def forward_with_cache(
        ...
        if store_cache:
            ...
            train_size = y_train.shape[1]
            y_train = y_train.unsqueeze(1).expand(-1, features.shape[1], *([-1] * (y_train.dim() - 1))) # (B, train_size)->(B, G+C, train_size)
        else:
