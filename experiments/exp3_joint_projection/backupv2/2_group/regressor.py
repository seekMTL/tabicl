...
class TabICLRegressor(RegressorMixin, TabICLBaseEstimator):
    ...
    num_outputs : int, default=1
        Number of output dimensions. When > 1, the regressor predicts multiple
        target dimensions simultaneously (e.g., [reward, delta_state_0, ..., delta_state_10]
        for RL tasks). ColEmbedding uses joint multi-target projection
        (``Linear(num_outputs, embed_dim)``) so the Set Transformer learns joint
        column attention patterns. The ICL stage uses per-dimension y_encoders
        and decoders for independent prediction of each output dimension.
    ...
    def _load_model(self) -> None:
        ...
        When num_outputs > 1, the pretrained single-output weights are mapped to
        the multi-output architecture:
          - col_embedder.y_encoder: Linear(1,128) → Linear(num_outputs,128)
            with weight repeat + scaling
          - icl_predictor.y_encoders/decoders: copied to all num_outputs heads
        ...
        if self.num_outputs > 1:
            # ── 多输出模式：ColEmbedding.y_encoder 需扩展，其余直接加载 ──
            self.model_ = TabICL(**config, num_outputs=self.num_outputs)
            self.model_config_ = {**config, "num_outputs": self.num_outputs}

            # 加载所有权重，跳过 col_embedder.y_encoder（形状不匹配，手动处理）
            model_state = self.model_.state_dict()
            for k, v in state_dict.items():
                if k == 'col_embedder.y_encoder.weight' or k == 'col_embedder.y_encoder.bias':
                    continue
                if k in model_state:
                    model_state[k].copy_(v)

            # 分组建模: reward(dim 0) 单独 y_encoder_reward，delta(dims 1..) 共享 y_encoder_delta
            # y_encoder_reward: Linear(1,128) ← 直接复制预训练权重，保留完整信号
            # y_encoder_delta:  Linear(num_outputs-1,128) ← repeat + scale，11 个 delta 量级接近无淹没
            cw = state_dict['col_embedder.y_encoder.weight']  # (128, 1)
            cb = state_dict['col_embedder.y_encoder.bias']    # (128,)
            self.model_.col_embedder.y_encoder_reward.weight.data.copy_(cw)
            self.model_.col_embedder.y_encoder_reward.bias.data.copy_(cb)
            self.model_.col_embedder.y_encoder_delta.weight.data.copy_(
                cw.repeat(1, self.num_outputs - 1) * (1.0 / (self.num_outputs - 1)))
            self.model_.col_embedder.y_encoder_delta.bias.data.copy_(cb)
        else:
            # ── 单输出模式：原始加载逻辑 ──
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> TabICLRegressor:
        ...
        1. Scaling target values using StandardScaler (per-dimension for multi-output)
        ...
        y : array-like of shape (n_samples,) or (n_samples, num_outputs)
            Training target values. For multi-output (num_outputs > 1),
            y must be 2D with shape (n_samples, num_outputs).
        ...
        # Handle y shape based on num_outputs
        if self.num_outputs > 1:
            # 多输出模式：y 应为 2D (n_samples, num_outputs)
            if y.ndim == 1:
                raise ValueError(
                    f"For multi-output (num_outputs={self.num_outputs}), y must be 2D "
                    f"with shape (n_samples, {self.num_outputs}), but got 1D array."
                )
            if y.shape[1] != self.num_outputs:
                raise ValueError(
                    f"y has {y.shape[1]} columns, but num_outputs={self.num_outputs}."
                )
        else:
            # 单输出模式：兼容列向量 y (n, 1) → (n,)
        ...
        # Scale target values
        # StandardScaler 原生支持 2D 输入：(n, d) → 逐列标准化
        if self.num_outputs > 1:
            self.y_scaler_ = StandardScaler()
            y_scaled = self.y_scaler_.fit_transform(y) # (n, num_outputs)
        else:
        ...
        # EnsembleGenerator 存储 y_scaled 供 transform 使用；多输出时 y_scaled 为 2D
        self.ensemble_generator_.fit(X, y_scaled) # fit过程：为每种归一化方法拟合预处理器（记录训练数据的统计量），供后续 transform() 使用

        self.model_kv_cache_ = None
        if self.kv_cache:
            if self.num_outputs > 1:
                # 多输出模式下 KV cache 暂不支持：每个维度需要独立的 y_emb 注入，
                # 对应不同的 KV 投影，cache 机制需要适配
                raise NotImplementedError(
                    "KV cache is not supported for multi-output (num_outputs > 1). "
                    "Set kv_cache=False when using num_outputs > 1."
                )
    ...
    def predict(
        ...
        Returns
        -------
        np.ndarray of shape (n_samples,) or (n_samples, num_outputs), or dict
            - Single-output (num_outputs=1): shape ``(n_samples,)`` if output_type
              is ``"mean"`` or ``"median"``, or ``(n_samples, n_quantiles)`` if
              output_type is ``"quantiles"``.
            - Multi-output (num_outputs>1): shape ``(n_samples, num_outputs)`` if
              output_type is ``"mean"`` or ``"median"``, or
              ``(n_samples, num_outputs, n_quantiles)`` if output_type is
              ``"quantiles"``.
        ...
        for key in output_type:
            ...
            if self.num_outputs > 1:
                # ── 多输出：反标准化 + 集成平均 ──
                if arr.ndim == 3:
                    # mean/median: (n_estimators, n_samples, num_outputs)
                    n_samples, n_out = arr.shape[1], arr.shape[2]
                    # reshape → (n_estimators*n_samples, num_outputs) → inverse → reshape back
                    arr = self.y_scaler_.inverse_transform(
                        arr.reshape(-1, n_out)).reshape(n_estimators, n_samples, n_out)
                    final_results[key] = np.mean(arr, axis=0)  # (n_samples, num_outputs)
                else:
                    # quantiles: (n_estimators, n_samples, num_outputs, n_quantiles)
                    n_samples, n_out, n_quantiles = arr.shape[1], arr.shape[2], arr.shape[3]
                    arr = self.y_scaler_.inverse_transform(
                        arr.transpose(0, 1, 3, 2).reshape(-1, n_out)
                    ).reshape(n_estimators, n_samples, n_quantiles, n_out).transpose(0, 1, 3, 2)
                    final_results[key] = np.mean(arr, axis=0)  # (n_samples, num_outputs, n_quantiles)
            else:
                # ── 单输出：原始逻辑 ──
