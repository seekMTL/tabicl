...
class ICLearning(nn.Module):
    ...
    def _icl_predictions(self, R: Tensor, y_train: Tensor) -> Tensor:
        ...
        y_train : Tensor
            ...
            (B, train_size, num_outputs) for multi-output, where train_size is the
        ...
        if self.num_outputs == 1:
            ...
            # R is shared across all dimensions (FiLM already encoded full target in ColEmbedding)
            assert not self.max_classes > 0, "Multi-output only supported for regression."
            outs = []
            for j in range(self.num_outputs):
                R_j = R.clone()
                y_j = y_train[:, :, j]  # (B, train_size)
                Ry_train = self.y_encoder(y_j.unsqueeze(-1))  # 共享 y_encoder
