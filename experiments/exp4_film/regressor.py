...
class TabICLRegressor(RegressorMixin, TabICLBaseEstimator):
    ...
    def _load_model(self) -> None:
        ...
        if self.num_outputs > 1:
            ...
            # FiLM 权重初始化: β 从预训练 y_encoder 继承，γ 零初始化
            cw = state_dict['col_embedder.y_encoder.weight']  # (128, 1)
            cb = state_dict['col_embedder.y_encoder.bias']    # (128,)
            film = self.model_.col_embedder.film

            # fc_gamma: 零初始化 → γ=0 (无乘法调制，预留 fine-tune 接口)
            torch.nn.init.zeros_(film.fc_gamma.weight)
            torch.nn.init.zeros_(film.fc_gamma.bias)

            # fc_beta: Linear(12→128)。reward(dim0) 完整预训练权重, delta(dims 1..) 缩放
            film.fc_beta.weight.data[:, 0] = cw.squeeze()
            film.fc_beta.weight.data[:, 1:] = cw.squeeze().unsqueeze(1) * (1.0 / (self.num_outputs - 1))
            film.fc_beta.bias.data.copy_(cb)
        else:
            # ── 单输出：原始加载逻辑 ──
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> TabICLRegressor:
        ...
        if self.kv_cache: # kv cache 可选加速
            if self.num_outputs > 1:
                # 多输出模式暂不支持 KV cache（每维度独立 y_emb 注入需不同 KV 投影）
