...
def validate_data(
    ...
    **check_params : kwargs
    ...
    # Automatically detect multi-output y for sklearn compatibility
    if not no_val_y and y is not None and not isinstance(y, str):
        y_arr = np.asarray(y)
        if y_arr.ndim > 1 and y_arr.shape[1] > 1:
            check_params = {**check_params, "multi_output": True}
