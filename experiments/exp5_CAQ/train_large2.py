"""CAQ-TabICL 共享模型训练：一个 CAQ 模块跨 12 维训练，产出一个 ICL 模型

与 train_large.py 的区别：
  - train_large.py：每维独立创建 CAQ 模型（12 组 action_encoder + causal_block 参数）
  - 本脚本：一个共享 CAQ 模块，每步预测全部 12 维，累积 loss 后一次反向传播

用法:
  python train_large2.py --epoch_idx 280 --lr 1e-4 --n_epochs 300 --device cuda
  python train_large2.py --resume checkpoints/bkup_shared
"""
...
def preprocess_data(X_state_tr, X_state_te, y_tr, y_te,
    ...
    """预处理 Hopper 数据，对齐 TabICL 预训练的 standard_scaling。
...
def train_shared(
    ...
    每步：依次预测全部 12 维，累积 loss 后一次反向传播。
    每 epoch 包含 n_steps_per_dim 步，每步处理 12 个 mini-batch。
    评估间隔 eval_interval epoch 时评估全部维度。
    """
    if best_mse is None:
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for _ in range(n_steps_per_dim):
            ...
            for d in range(OUTPUT_DIMS):
                # 返回的X_state, X_action, yb均[1, n_context+n_query, ...]形状，即截取X_state_tr, X_action_tr, y_tr部分
                X_state, X_action, yb = create_icl_batch(
                    X_state_tr, X_action_tr, y_tr[:, d], n_context, n_query, device
                )
            ...
            total_loss.backward() # 12 维 loss 累积后一次反向传播
            optimizer.step()
...
def main():
    parser = argparse.ArgumentParser(description="CAQ-TabICL 共享模型训练")
    ...
    parser.add_argument("--n_steps_per_dim", type=int, default=50,
                        help="每 epoch 的优化步数，每步累积全部 12 维 loss 后一次 backward")
    ...
    print(f"\n{'='*60}")
    print(f"  🧬 共享 CAQ 模型训练（12 维 loss 累积后一次反向传播）")
    ...
    # ── 训练完成后统一重新评估全部 12 维（不依赖训练中 eval_interval 的快照）──
    mse_caq_final = None
