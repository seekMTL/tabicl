"""CAQ-TabICL 通用 RL 环境训练脚本（并行 ICL 推理）。

通过 --env 指定 RL 环境，自动推断 state/action/output 维度，无需手动设置。
支持环境：Hopper, HalfCheetah, Walker2d, Ant

用法:
  python train_large3_walker2d.py --env Hopper --epoch_idx 280 --lr 1e-4 --device cuda
  python train_large3_walker2d.py --env Walker2d --epoch_idx 300 --device cuda
  python train_large3_walker2d.py --resume checkpoints/xxx
"""
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  🧬 共享 CAQ 模型训练（{OUTPUT_DIMS} 维 loss 累积后一次反向传播）")
