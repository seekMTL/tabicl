"""CAQ-TabICL 混合训练（action 作为特征）：多环境 RL 数据 + SCM replay 防遗忘。

在 train_large4_mixed2.py 基础上的关键变更（Ant 诊断实验 E5 的结论落地）:
  - CAQ-only 训练 Ant 时 dims 20-27 劣化 12~18 倍 → action 经 CLS 调制的
    信息流对 action 驱动维度失效
  - 修复: action 作为普通特征列拼入 state 一起进 ColEmbedding
    （与 baseline TabICLRegressor 的信息流一致），z-score 同管线处理
  - action_encoder + causal_block 保留为额外增强路径（可消融）
...
设计要点（继承 mixed2）:
  ...
  - SCM replay: X_action=None → 零动作（causal 路径零初始化退化为原生行为）
    SCM 表格不加 action 列，前向即纯预训练计算
  - action_dim 统一为 MAX_ACTION_DIM=8，低维 action 右侧补零（对 Linear 无害）
    特征表中 action 列同样补零对齐

用法:
  # Ant 单环境验证（与 ant_v4_single 对照，唯一变量为 action 通道）
  python train_large4_actionfeat.py --envs "Ant-v4" --device cuda
  # 4 环境混合
  python train_large4_actionfeat.py --envs "Hopper,Walker2d,HalfCheetah,Ant-v4" \\
      --n_context 3000 --env_block_size 1 --device cuda
"""
