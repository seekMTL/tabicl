...
用法:
  python train_large4_mixed.py --envs "Hopper,Walker2d,HalfCheetah,Ant-v4" --lr 1e-5 --device cuda
  python train_large4_mixed.py --envs "Hopper" --rl_ratio 1.0          # 纯 RL，无 replay
  python train_large4_mixed.py --envs "Hopper" --freeze_col_embedder --freeze_native_blocks  # 等同 train_large3
  python train_large4_mixed.py --resume checkpoints/xxx
"""
...
def scm_train_step(model, scm_sampler, criterion, device):
    ...
    # SCM 前向的激活峰值较大（多特征 × batch），释放缓存避免碎片化累积
...
def train_mixed(
    ...
    每步：K 步 RL（环境轮流）→ 1 步 SCM replay（K = round(1/(1-rl_ratio)) - 1）。
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for _ in range(n_steps_per_epoch):
            ...
            if is_scm_step:
                ...
                env_name = env_names[rl_step_count % len(env_names)]
                rl_step_count += 1
                loss = rl_train_step(
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  🧬 混合训练（{len(env_names)} 环境轮流，rl_ratio={args.rl_ratio}）")
    ...
    print(f"  {'Env':<12} {'CAQ avg':>12} {'Base avg':>12} {'Ratio':>10}")
    print(f"  {'-'*50}")
    overall_c, overall_b = [], []
    for env_name in env_names:
        c = np.mean(list(mse_all[env_name].values()))
        overall_c.append(c)
        if mse_base and env_name in mse_base:
            b = np.mean(mse_base[env_name])
            overall_b.append(b)
            print(f"  {env_name:<12} {format_mse(c):>12} {format_mse(b):>12} {c/b:>10.2f}x")
        else:
            print(f"  {env_name:<12} {format_mse(c):>12} {'-':>12} {'-':>10}")
    if overall_b and len(overall_b) == len(overall_c):
        print(f"  {'-'*50}")
        print(f"  {'OVERALL':<12} {format_mse(np.mean(overall_c)):>12} "
              f"{format_mse(np.mean(overall_b)):>12} {np.mean(overall_c)/np.mean(overall_b):>10.2f}x")
