"""CAQ-TabICL 混合训练 v5（action 作为特征 + target-aware）：多环境 RL 数据 + SCM replay。

在 train_large4_actionfeat.py 基础上的关键变更（评估拆解实验的结论落地）:
  - 评估拆解证实: target-aware（ColEmbedding 的 y 条件嵌入）是冻结 ICL 正常
    工作的必要条件——同一冻结模型 + action 特征表，关/开 target-aware 时
    Ant 分别为 9.19x / 1.01x（vs baseline）
  - 修复: 新增 --target_aware 开关（默认关，兼容旧 checkpoint）。
    开启时 rl_train_step 改用逐维 y 条件的批处理：同一张表堆叠 D 次
    （batch 维 = 输出维度数），每个 batch 元素用自己的 y 维度走 y_encoder
    路径——等效逐维前向，批处理并行
  - 预期: TA-on 下从头适配能否超过冻结基线（1.01x）是本实验的唯一裁决问题

设计要点（继承 actionfeat）:
  - action 作为普通特征列拼入 state 一起进 ColEmbedding（35 列，z-score 同管线）
  - action_encoder + causal_block 保留为额外增强路径
  ...
  - SCM replay: X_action=None → 零动作；SCM 表格不加 action 列，
    且 TA-on 下 SCM 前向与预训练完全同构（预训练即 target-aware）
  - action_dim 统一为 MAX_ACTION_DIM=8，低维 action 右侧补零

用法:
  # Ant 单环境 TA-on 适配（V1 裁决实验；col/row 阶段显存 ≈ D 倍，需大显存）
  python train_large5_targetaware.py --envs "Ant-v4" --target_aware --device cuda
  # 4 环境混合（TA-on）
  python train_large5_targetaware.py --envs "Hopper,Walker2d,HalfCheetah,Ant-v4" \\
      --target_aware --n_context 3000 --env_block_size 1 --device cuda
"""
...
def create_icl_batch_shared(X_state, X_action, y, n_context, n_query, device):
    ...
    action 作为普通特征列拼入 state（Xs_state 为 H+8 列表格），
    同时保留 X_action 供 action_encoder/causal_block 的增强路径。
    ...
    Xa_ctx = pad_action(X_action[ctx_idx])
    Xa_q = pad_action(X_action[q_idx])
    Xs_state = np.concatenate([
        np.concatenate([X_state[ctx_idx], Xa_ctx], axis=1),
        np.concatenate([X_state[q_idx], Xa_q], axis=1),
    ])
    Xs_action = np.concatenate([Xa_ctx, Xa_q])
    yb = np.stack(
...
def evaluate_env(model, env_data, device, criterion, n_context_eval):
    ...
    with torch.no_grad():
        for d in range(output_dims):
            # 与训练路径一致：action 拼入 state 作为特征（评估信息流必须对齐）
            Xa_ctx = pad_action(X_action_tr[:n_ctx])
            Xa_te = pad_action(X_action_te)
            eval_state = np.concatenate([
                np.concatenate([X_state_tr[:n_ctx], Xa_ctx], axis=1),
                np.concatenate([X_state_te, Xa_te], axis=1),
            ])
...
def main():
    ...
    train_args = {
        ...
        "action_as_features": True,  # 本脚本的标记：action 拼入特征表
        "target_aware": args.target_aware,
