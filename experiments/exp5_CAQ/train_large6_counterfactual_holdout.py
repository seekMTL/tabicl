"""CAQ-TabICL 混合训练 v6.1（反事实 fork 数据，flat 随机打乱版 + state-only + TA-on）：SCM replay。
hold-out 变体：--holdout 指定留出环境（不参与训练），训练中/结束时对其做
跨世界外推评估——zero-shot（context=同 body 其他训练环境池拼接）与
few-shot（context=自身训练池）双协议，与 baseline 同协议对比。

在 train_large5_taon_stateonly.py 基础上，将 RL 数据换成反事实 fork 数据
（2026-09-06 重新采集的 mujoco v4 版：
/home/lizitao/project/other/sample_collect/Counterfactual/mujoco_v4_260906/，
加噪 action 版；旧 logs_v4-old 为随机 action 版）。

数据组织完全仿照 train_large5（不再利用 fork 组结构）:
  - 加载后全行 rng.permutation 整体打乱 → 前 80% 训练、后 n_test 行测试
    （与 train5 完全一致：打乱后切分，训练/测试行序随机）
  - 训练表: 随机抽 n_context + n_query 行（create_icl_batch_flat，即
    train5 的 create_icl_batch_shared）；不再做「组内 ctx/query 拆分」
  - 评估: context = 训练池打乱后前 n_context_eval 行（随机样本），
    query = 测试集全部行；baseline 协议不变
  - 同 src 的 8 条 fork 打乱后分散各处：训练时同 src 行同表概率极低；
    评估时测试行与 context 同 src 的重叠概率 ~4%（加噪版组内 delta 差异
    仅 1.6% 总方差，泄漏影响有限；这是 flat 协议的固有代价）

v6.1 变更原因（v6 fork-aware 版崩坏 3.08x 的修复）:
  - fork-aware 组内拆分给训练开了「query 与 context 同 state」的复制捷径，
    评估协议无此捷径 → train/eval 错位
  - 行序保持采集顺序未打乱 → 评估 context 固定在采集最早 2,500 个 src
    （reward 均值 -1.17 vs 测试 +3.43，分布错位近 4σ），baseline 与评估
    双双被污染
  - flat 协议下 causal 通道（action_encoder → causal_block 逐行独立调制）
    仍可从每行 (s, a) → δ 的监督中学习，因果信号不依赖行排列

继承 train_large5 的设计要点:
  - target-aware 开启（冻结 ICL 正常工作的必要条件），默认 True
  - rl_train_step 逐维 y 条件批处理 + --ta_dim_chunk 维度分块
    （24GB 卡 Ant 28 维用 chunk=7）
  - 训练参数: ColEmbedding + RowInteraction(native blocks) + CAQ
    冻结: ICLearning（95.5% 参数）
  - 数据混合: 每 K 步 RL(fork) 数据 → 1 步 SCM 合成数据（80/20 比例）
  - SCM replay: X_action=None → 零动作，CAQ 退化为原生 RowInteraction，
    为 Col/Row 提供预训练分布锚点
  - action_dim 统一为 MAX_ACTION_DIM=8，低维 action 右侧补零

用法:
  # 冒烟测试（2 epoch × 5 steps）
  python train_large6_counterfactual.py --envs "Ant-v4" --ta_dim_chunk 7 \\
      --n_epochs 2 --n_steps_per_epoch 5 --eval_interval 1 --skip_baseline \\
      --save_dir checkpoints/train6_counterfactual/smoke
  # Ant 单环境正式训练
  python train_large6_counterfactual.py --envs "Ant-v4" --ta_dim_chunk 7 \\
      --device cuda --save_dir checkpoints/train6_counterfactual/antv4_ctx1024_v4noise_flat
"""
...
CF_DATA_BASE = "/home/lizitao/project/other/sample_collect/Counterfactual/mujoco_v4_260906"

# 环境配置：目录名 = 环境名（fork_samples.npz 在其下）。数据集共 10 环境，
# 目录名与 Gym 环境名一致。
ENV_CONFIGS = {
    "Hopper-v4":                {"dir": "Hopper-v4"},
    "Walker2d-v4":              {"dir": "Walker2d-v4"},
    "HalfCheetah-v4":           {"dir": "HalfCheetah-v4"},
    "Ant-v4":                   {"dir": "Ant-v4"},
    # "HumanoidStandup-v4":       {"dir": "HumanoidStandup-v4"},   # action=17d > MAX_ACTION_DIM，勿直接启用
    # "Humanoid-v4":              {"dir": "Humanoid-v4"},           # action=17d > MAX_ACTION_DIM，勿直接启用
    "InvertedDoublePendulum-v4": {"dir": "InvertedDoublePendulum-v4"},
    "InvertedPendulum-v4":      {"dir": "InvertedPendulum-v4"},
    "Reacher-v4":               {"dir": "Reacher-v4"},
    "Swimmer-v4":               {"dir": "Swimmer-v4"},
    # ── 扰动环境(perturbed envs,mbpo_jax-main/perturb_envs.py)────────
    # 目录名 = 注册 id 去掉 -v1 后缀。数据由 mbpo_jax-main 侧采集生成:
    #   python generate_perturbed_xmls.py
    #   python main_mbpo.py --env_name <slug>-v1 [--seed S]   # 产出 env_buffer.npz
    #   python fork_env.py --env_name <slug>-v1 --buffer .../env_buffer.npz
    #   mkdir -p <CF_DATA_BASE>/<slug> && cp fork_samples.npz <CF_DATA_BASE>/<slug>/
    # 训练通过 --envs 显式指定环境列表,此处仅为可选白名单:
    # 未落盘的条目不写进 --envs 即可,无需注释。
    "HopperGravity-g-1":        {"dir": "HopperGravity-g-1"},
    "HopperGravity-g-0.5":      {"dir": "HopperGravity-g-0.5"},
    "HopperGravity-g0":         {"dir": "HopperGravity-g0"},
    "HopperGravity-g0.5":       {"dir": "HopperGravity-g0.5"},
    "HopperGravity-g1":         {"dir": "HopperGravity-g1"},
    "Walker2dFriction-f0.3":    {"dir": "Walker2dFriction-f0.3"},
    "Walker2dFriction-f1":      {"dir": "Walker2dFriction-f1"},
    "Walker2dFriction-f3":      {"dir": "Walker2dFriction-f3"},
    "HalfCheetahSlope-s-10":    {"dir": "HalfCheetahSlope-s-10"},
    "HalfCheetahSlope-s0":      {"dir": "HalfCheetahSlope-s0"},
    "HalfCheetahSlope-s10":     {"dir": "HalfCheetahSlope-s10"},
    "AntJoints-af0":            {"dir": "AntJoints-af0"},
    "AntJoints-af1":            {"dir": "AntJoints-af1"},
    "AntJoints-af2":            {"dir": "AntJoints-af2"},
    "AntJoints-af3":            {"dir": "AntJoints-af3"},
    "AntJoints-af4":            {"dir": "AntJoints-af4"},
    "AntJoints-af5":            {"dir": "AntJoints-af5"},
    "AntJoints-af6":            {"dir": "AntJoints-af6"},
    "AntJoints-af7":            {"dir": "AntJoints-af7"},
}

# action_dim 上限：除 Humanoid 两环境（17d）外最大为 8（Ant），低维 action
# 右侧补零。补零对 Linear 层（加权和）无害：零输入不贡献、不稀释。
# 若启用 Humanoid：需把上限提到 17 并重建 action_encoder（且其 376d state
# → y 377 维，TA-on 显存需求剧增，需大幅调小 ta_dim_chunk）。
MAX_ACTION_DIM = 8

# fork_env.py 的 --num_actions 默认值：每个 src 采集的 fork 数（组大小）。
# flat 协议下不再利用组结构，此常量仅用于文档说明数据语义。
FORKS_PER_SRC = 8
...
def load_counterfactual_data(env_dir: str):
    """加载 fork_samples.npz：state/action/next_state/reward。

    y = [reward, next_state - state]（多输出，与之前实验一致）。
    flat 协议：不读取 src_idx，全行打乱后按行切分（仿 train_large5）。
    """
    data = np.load(f"{CF_DATA_BASE}/{env_dir}/fork_samples.npz")
...
def create_icl_batch_flat(X_state, X_action, y, n_context, n_query, device):
    """创建 flat ICL batch（仿 train_large5 的 create_icl_batch_shared）。

    全行随机 permutation 抽 n_context + n_query 行——不利用 fork 组结构。
    同 src 的 8 条 fork 打乱后分散各处，训练表内基本无重复 state
    （同 src 两行同表概率 ~1600×8/n_tr，可忽略）。

    state-only 变体：action 不拼入特征表（Xs_state 仅 H 列 state），
    action 只走 row_interactor 的 action_encoder → causal_block 调制路径。
    causal 通道是逐行独立的（B*T 展平、行间无交互），其监督信号 =
    每行 (s, a) → δ，不依赖同 src 行是否相邻。

    Returns:
        Xs_state:  (1, T, H)  T = n_context + n_query
        Xs_action: (1, T, 8)
        yb:        (OUTPUT_DIMS, T)
    """
    n_total = len(X_state)
    ...
    Xs_state = np.concatenate([X_state[ctx_idx], X_state[q_idx]])  # (T, H)
    Xs_action = pad_action(np.concatenate([X_action[ctx_idx], X_action[q_idx]]))
    yb = np.stack(
...
def evaluate_env(model, env_data, device, criterion, n_context_eval):
    """评估单个环境（flat 协议，仿 train_large5）：context = 训练池打乱后
    前 n_context_eval 行（随机样本，与训练表分布一致），query = 测试集全部行。

    返回 {dim: MSE(原始尺度)} 与平均值。y_stats 用于将标准化空间的预测
...
def evaluate_baseline_full(X_tr, X_te, y_tr, y_te, device="cuda", n_context_eval=100000):
    ...
    上下文取训练池前 n_ctx 行（训练池已全行打乱 → 随机样本）。
    """
...
# ═══════════════════════════════════════════════════════════════════
# hold-out 外推评估（zero-shot 跨环境 context）
# ═══════════════════════════════════════════════════════════════════

def body_of(env_name):
    """环境名 → body 前缀（同 body 环境共享特征语义）：
    "HopperGravity-g1" → "HopperGravity"，"AntJoints-af4" → "AntJoints"。
    """
    return env_name.split("-")[0]


def evaluate_env_cross(model, holdout_data, ctx_datas, device, criterion,
                       n_context_eval):
    """hold-out zero-shot 评估（CAQ）：context = 同 body 各训练环境池拼接，
    query = hold-out 环境测试行。逐维前向（与 evaluate_env 相同的
    state-only 路径）。

    跨环境 context 的 y 为各环境自身 z-score（同 body 环境分布相近），
    预测用 hold-out 的 y_stats 逆变换到原始尺度与 raw 目标比较——
    CAQ 与 baseline 同协议，公平。
    """
    X_state_te = holdout_data["X_state_te_pp"]
    X_action_te = holdout_data["X_action_te"]
    y_te_pp = holdout_data["y_te_pp"]
    y_te_raw = holdout_data["y_te_raw"]
    y_stats = holdout_data["y_stats"]
    output_dims = holdout_data["output_dims"]

    n_ctx_per = max(1, n_context_eval // len(ctx_datas))
    ctx_state = np.concatenate([d["X_state_tr_pp"][:n_ctx_per] for d in ctx_datas])
    ctx_action = np.concatenate([d["X_action_tr"][:n_ctx_per] for d in ctx_datas])
    ctx_y = np.concatenate([d["y_tr_pp"][:n_ctx_per] for d in ctx_datas])
    n_ctx = len(ctx_state)
    ...
    with torch.no_grad():
        for d in range(output_dims):
            eval_state = np.concatenate([ctx_state, X_state_te])
            eval_action = pad_action(np.concatenate([ctx_action, X_action_te]))
            eval_y = np.concatenate([ctx_y[:, d], y_te_pp[:, d]])
            ...
            mean, std = y_stats[d]
            pred = pred * std + mean
            target = torch.from_numpy(y_te_raw[:, d]).float().to(device)
            mse_dims[d] = criterion(pred, target).item()

    return mse_dims


def evaluate_baseline_cross(ctx_list, X_te, y_te, device="cuda", n_context_eval=100000):
    """Baseline zero-shot：context 拼接自多个同 body 训练环境（raw state+action
    拼接表，与 evaluate_baseline_full 相同的 TabICLRegressor 协议）。
    """
    n_ctx_per = max(1, n_context_eval // len(ctx_list))
    X_ctx = np.concatenate([X[:n_ctx_per] for X, _ in ctx_list])
    y_ctx = np.concatenate([y[:n_ctx_per] for _, y in ctx_list])
    mse = np.zeros(y_te.shape[1])
...
def save_progress(save_dir, mse_all, mse_base, epoch, scm_loss, forget_score,
                  scm_loss_baseline, elapsed, holdout_mse=None):
    save_dir.mkdir(parents=True, exist_ok=True)
    progress = {
        "epoch": epoch,
        "mse_caq": {env: {str(d): float(v) for d, v in mse.items()} for env, mse in mse_all.items()},
        ...
        "mse_holdout": {k: {str(d): float(v) for d, v in mse.items()}
                        for k, mse in holdout_mse.items()} if holdout_mse else {},
...
def rl_train_step(model, env_data, criterion, device, n_context, n_query,
    ...
    """RL(fork) 训练步（flat 协议）：随机抽 n_context + n_query 行，
    逐维 MSE 求和后取平均。

    表结构：n_context 行 context + n_query 行 query（全行随机，无组结构）。

    两种模式（继承 train_large5）：
    - target_aware=False：ColEmbedding+RowInteraction 只做一次，表示跨维度
      共享，ICL 阶段 repeat 全维并行。
    - target_aware=True：恢复预训练原生的 y 条件列嵌入，同一张表堆叠
      OUTPUT_DIMS 次（batch 维），每个 batch 元素用自己的 y 维度走
      y_encoder 路径——等效于逐维前向，批处理并行。

    显存与分块（ta_dim_chunk）：TA-on 全维批处理（B=D）的 col/row 激活
    ≈ D 倍，Ant 28 维需分块（24GB 卡 chunk=7）。分块时每块独立前向 +
    立即反向（do_backward=True），梯度按块权重累积——等效全维一次。
    """
    X_state, X_action, yb_all = create_icl_batch_flat(
    ...
    def _forward(dim_idxs):
        ...
        X_state_b = X_state.repeat(len(dim_idxs), 1, 1)   # (k, T, H)
...
def train_mixed(
    ...
    holdout_datasets=None, holdout_base=None,
):
    """混合训练主循环。

    每步：K 步 RL(fork flat)（环境块状轮流）→ 1 步 SCM replay
    （K = round(1/(1-rl_ratio)) - 1）。
    环境块状轮流：同一环境连续 env_block_size 个 RL 步再切换，减少环境间
    梯度交替更新造成的相互干扰（v4 中 Ant 2.4x 的头号嫌疑）。
    每 eval_interval epoch：评估各环境 MSE + SCM 遗忘监控；
    若有 hold-out 环境：同时评估其 zero-shot / few-shot 外推 MSE。
    """
    ...
    holdout_mse = {}    # 最近一次 hold-out 评估结果（{env:zero/few: mse_dims}）
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        if (epoch + 1) % eval_interval == 0 or epoch == start_epoch:
            ...
            # ── hold-out 外推评估（zero-shot + few-shot）──
            holdout_mse = {}
            for h_name, h_data in holdout_datasets.items():
                h_body = body_of(h_name)
                ctx_datas = [env_datasets[e] for e in env_names
                             if body_of(e) == h_body]
                holdout_mse[f"{h_name}:zero"] = evaluate_env_cross(
                    model, h_data, ctx_datas, device, criterion, n_context_eval)
                holdout_mse[f"{h_name}:few"] = evaluate_env(
                    model, h_data, device, criterion, n_context_eval)
            ...
            for h_name in holdout_datasets:
                z = holdout_mse[f"{h_name}:zero"]
                f = holdout_mse[f"{h_name}:few"]
                za, fa = np.mean(list(z.values())), np.mean(list(f.values()))
                hb = holdout_base.get(h_name, {})
                zb = np.mean(hb["zero"]) if "zero" in hb else float("nan")
                fb = np.mean(hb["few"]) if "few" in hb else float("nan")
                print(f"    [HOLD:{h_name:<12}] zero={format_mse(za)} (base {format_mse(zb)}) "
                      f"| few={format_mse(fa)} (base {format_mse(fb)})")
    ...
    return mse_all, best_mse, holdout_mse
...
def main():
    parser = argparse.ArgumentParser(description="CAQ-TabICL 混合训练（反事实 fork 数据 + SCM replay）")
    # 环境
    parser.add_argument("--envs", type=str, default="Ant-v4",
                        help=f"环境列表（逗号分隔），可选: {list(ENV_CONFIGS.keys())}")
    parser.add_argument("--holdout", type=str, default="",
                        help="留出环境（逗号分隔，需在 --envs 中）：不参与训练，仅做 "
                             "hold-out 外推评估。每个留出环境跑两种协议："
                             "zero-shot（context=同 body 其他训练环境池各行拼接）与 "
                             "few-shot（context=自身训练池前 N 行，复用 evaluate_env）")
    # 训练
    parser.add_argument("--lr", type=float, default=1e-5)
    parser.add_argument("--n_context", type=int, default=1024,
                        help="训练表 context 行数（仿 train_large5）")
    parser.add_argument("--n_query", type=int, default=512,
                        help="训练表 query 行数（仿 train_large5）")
    parser.add_argument("--n_context_eval", type=int, default=20000,
                        help="评估时 ICL 上下文行数（训练池打乱后的前 N 行）")
    ...
    parser.add_argument("--target_aware", action=argparse.BooleanOptionalAction, default=True,
                        help="启用 ColEmbedding 的 target-aware y 条件嵌入（默认开，评估实验"
                             "证实其为冻结 ICL 正常工作的必要条件；可用 --no-target_aware 关）")
    ...
    parser.add_argument("--save_dir", type=str, default="checkpoints/train6_counterfactual")
    ...
    holdout_names = [e.strip() for e in args.holdout.split(",") if e.strip()]
    for h in holdout_names:
        assert h in env_names, f"hold-out 环境 {h} 不在 --envs 中"
    train_env_names = [e for e in env_names if e not in holdout_names]

    device = torch.device(args.device if torch.cuda.is_available() else "cpu")
    save_dir = Path(args.save_dir) if args.save_dir else None

    # ── 加载反事实数据 + 全行随机打乱 + 切分（仿 train_large5）──
    print("📦 加载反事实 fork 数据（fork_samples.npz，大文件解压需 1-3 分钟）...")
    env_datasets = {}
    for env_name in env_names:
        cfg = ENV_CONFIGS[env_name]
        X_state, X_action, y = load_counterfactual_data(cfg["dir"])

        # 全行整体打乱（与 train_large5 完全一致：rng.permutation 后切分）
        ...
        env_datasets[env_name] = {
            ...
            # action 已 z-score（与 state 同管线），供 action_encoder 使用
        ...
        print(f"  [{env_name:<12}] state={X_state.shape[1]}d action={X_action.shape[1]}d "
              f"target={y.shape[1]}d | 训练 {len(X_state_tr):,} 行 测试 {len(X_state_te):,} 行")
    ...
    if not args.skip_baseline:
        ...
        # hold-out 环境的 baseline：zero-shot（跨环境 context）+ few-shot（自身池）
        if holdout_names:
            print(f"\n📊 Hold-out Baseline 评估中（zero-shot + few-shot）...")
            for h_name in holdout_names:
                hd = env_datasets[h_name]
                h_body = body_of(h_name)
                ctx_list = [(env_datasets[e]["X_tr_raw"], env_datasets[e]["y_tr_raw"])
                            for e in train_env_names if body_of(e) == h_body]
                z = evaluate_baseline_cross(
                    ctx_list, hd["X_te_raw"], hd["y_te_raw"],
                    device=str(device), n_context_eval=args.n_context_eval)
                f = evaluate_baseline_full(
                    hd["X_tr_raw"], hd["X_te_raw"], hd["y_tr_raw"], hd["y_te_raw"],
                    device=str(device), n_context_eval=args.n_context_eval)
                holdout_base[h_name] = {"zero": z, "few": f}
                print(f"  [{h_name:<12}] zero={format_mse(np.mean(z))} "
                      f"few={format_mse(np.mean(f))}")
    ...
    scheduler = CosineAnnealingLR(optimizer, T_max=args.n_epochs, eta_min=args.lr * 0.05)
    ...
    train_args = {
        ...
        "freeze_col_embedder": args.freeze_col_embedder,
        "freeze_native_blocks": args.freeze_native_blocks,
        "envs": train_env_names, "scm_batch_size": args.scm_batch_size,
        "env_block_size": args.env_block_size,
        "scm_loss_weight": args.scm_loss_weight,
        "holdout": holdout_names,  # 留出环境（不参与训练，仅外推评估）
        "data": "counterfactual_v4noise_flat",  # 加噪反事实数据 + flat 全行打乱协议
    ...
    print(f"\n{'='*60}")
    print(f"  🧬 反事实混合训练（{len(train_env_names)} 训练环境块状轮流 block={args.env_block_size}，"
          f"rl_ratio={args.rl_ratio}，SCM 权重={args.scm_loss_weight}）")
    if holdout_names:
        print(f"  🔬 hold-out 留出环境（不参与训练，仅外推评估）: {', '.join(holdout_names)}")
    print(f"  flat 表（全行随机）: {args.n_context} ctx + {args.n_query} query 行")
    ...
    t0 = time.time()
    mse_all = None
    holdout_mse_last = None
    env_datasets_train = {e: env_datasets[e] for e in train_env_names}
    holdout_datasets = {e: env_datasets[e] for e in holdout_names}
    try:
        mse_all, best_mse_final, holdout_mse_last = train_mixed(
            caq, optimizer, scheduler, criterion,
            env_datasets_train, scm_sampler,
            ...
            holdout_datasets=holdout_datasets, holdout_base=holdout_base,
        )
    ...
    print(f"\n{'='*70}")
    print(f"  📊 最终对比（{len(mse_all)} 训练环境）")
    print(f"  {'Env':<22} {'CAQ last':>12} {'CAQ best':>12} {'Base':>12} {'best/Base':>10} {'last/Base':>10}")
    print(f"  {'-'*74}")
    ...
    for env_name in train_env_names:
        ...
        if mse_base and env_name in mse_base:
            ...
            overall_b.append(b)
            print(f"  {env_name:<22} {format_mse(c_last):>12} {format_mse(c_best):>12} "
    ...
    if overall_b and len(overall_b) == len(overall_c):
        print(f"  {'-'*74}")
        print(f"  {'OVERALL':<22} {format_mse(np.mean(overall_c)):>12} {format_mse(np.mean(overall_best)):>12} "
    ...
    # ── hold-out 外推对比 ──
    if holdout_names and holdout_mse_last:
        print(f"\n{'='*86}")
        print(f"  📊 Hold-out 外推对比（{len(holdout_names)} 留出环境，模型从未见过）")
        print(f"  {'Env':<22} {'CAQ zero':>12} {'Base zero':>12} {'z/Base':>8} "
              f"{'CAQ few':>12} {'Base few':>12} {'f/Base':>8}")
        print(f"  {'-'*82}")
        for h_name in holdout_names:
            z = holdout_mse_last[f"{h_name}:zero"]
            f = holdout_mse_last[f"{h_name}:few"]
            za, fa = np.mean(list(z.values())), np.mean(list(f.values()))
            hb = holdout_base.get(h_name, {})
            if "zero" in hb:
                zb, fb = np.mean(hb["zero"]), np.mean(hb["few"])
                print(f"  {h_name:<22} {format_mse(za):>12} {format_mse(zb):>12} {za/zb:>8.2f}x "
                      f"{format_mse(fa):>12} {format_mse(fb):>12} {fa/fb:>8.2f}x")
            else:
                print(f"  {h_name:<22} {format_mse(za):>12} {'-':>12} {'-':>8} "
                      f"{format_mse(fa):>12} {'-':>12} {'-':>8}")
